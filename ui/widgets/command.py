import os
import hashlib
import shlex
import time

from PySide6.QtGui import QColor
from PySide6.QtWidgets import (
    QHBoxLayout, QLabel, QLineEdit,
    QPushButton, QTreeWidget, QTreeWidgetItem,
    QVBoxLayout, QWidget,
)

from ui.resources.constants import (
    ACCENT_GREEN, ACCENT_ORANGE, ACCENT_RED,
    TEXT_PRIMARY, TEXT_SECONDARY, TEXT_TERTIARY,
)
from utils.state import AppState


class GitCommandParser:
    """Parses 'git <verb> [flags] [args]' into a structured dict.

    The leading 'git' token is optional. Returns None on parse failure.
    """

    SUPPORTED = {
        "commit":   "Create a new commit from staged changes",
        "add":      "Stage file(s) to the index",
        "checkout": "Move HEAD to a ref or restore files",
        "switch":   "Move HEAD to a branch (modern syntax)",
        "reset":    "Move branch tip and optionally index / working directory",
        "branch":   "Create, list, or delete a branch",
        "tag":      "Create a lightweight or annotated tag",
        "revert":   "Create an inverse commit to undo changes",
        "merge":    "Merge a branch into the current branch",
        "stash":    "Save and restore dirty working-directory state",
    }

    @classmethod
    def parse(cls, raw: str) -> "dict | None":
        try:
            tokens = shlex.split(raw.strip())
        except ValueError:
            return None

        if not tokens:
            return None
        if tokens[0].lower() == "git":
            tokens = tokens[1:]
        if not tokens:
            return None

        verb = tokens[0].lower()
        flags, args, i = {}, [], 1

        while i < len(tokens):
            t = tokens[i]
            if t.startswith("-"):
                if i + 1 < len(tokens) and not tokens[i + 1].startswith("-"):
                    flags[t] = tokens[i + 1]
                    i += 2
                else:
                    flags[t] = True
                    i += 1
            else:
                args.append(t)
                i += 1

        return {
            "original": raw.strip(),
            "verb":     verb,
            "flags":    flags,
            "args":     args,
            "known":    verb in cls.SUPPORTED,
        }


class GitCommandPredictor:
    """Predicts what internal git changes a command would produce.

    Entirely read-only. Each verb handler returns a prediction dict with keys:
    `{command, verb, supported, interpretation, objects, refs, index, head, warnings, note}`.
    """

    def predict(self, repo, parsed: dict) -> dict:
        if repo is None:
            return self._err(parsed, "No repository loaded.")
        if not parsed.get("known"):
            known = ", ".join(GitCommandParser.SUPPORTED)
            return self._err(parsed, f"'{parsed['verb']}' is not simulated.\nSupported: {known}")

        handler = getattr(self, f"_{parsed['verb']}", self._checkout)
        try:
            return handler(repo, parsed)
        except Exception as exc:
            return self._err(parsed, f"Prediction error: {exc}")

    @staticmethod
    def _base(parsed: dict) -> dict:
        return {
            "command":        parsed["original"],
            "verb":           parsed["verb"],
            "supported":      True,
            "interpretation": "",
            "objects":        [],
            "refs":           [],
            "index":          [],
            "head":           {},
            "warnings":       [],
            "note":           "",
        }

    @staticmethod
    def _obj(type_, action, name, sha_before="", sha_after="", detail="") -> dict:
        return {
            "type":       type_,
            "action":     action,
            "name":       name,
            "sha_before": sha_before,
            "sha_after":  sha_after,
            "detail":     detail,
        }

    @staticmethod
    def _err(parsed: dict, msg: str) -> dict:
        return {
            "command":        parsed.get("original", ""),
            "verb":           parsed.get("verb", ""),
            "supported":      False,
            "interpretation": msg,
            "objects":        [],
            "refs":           [],
            "index":          [],
            "head":           {},
            "warnings":       [],
            "note":           "",
        }

    @staticmethod
    def _author(repo) -> str:
        try:
            cfg = repo.config_reader()
            return (
                f"{cfg.get_value('user', 'name', 'Unknown')}"
                f" <{cfg.get_value('user', 'email', '?')}>"
            )
        except Exception:
            return "Unknown User <unknown@example.com>"

    @staticmethod
    def _approx_sha(type_: str, tree: str, parent: str,
                    author: str, message: str, extra_parent: str = "") -> str:
        """Compute an approximate git object SHA using git's own header format."""
        ts   = int(time.time())
        body = f"tree {tree}\n"
        if parent:
            body += f"parent {parent}\n"
        if extra_parent:
            body += f"parent {extra_parent}\n"
        body += f"author {author} {ts} +0000\ncommitter {author} {ts} +0000\n\n{message}"
        header = f"{type_} {len(body.encode())}\x00"
        return hashlib.sha1((header + body).encode("utf-8", errors="replace")).hexdigest()

    @staticmethod
    def _head_info(repo) -> tuple:
        """Returns (head_sha, tree_sha, branch_name, is_detached)."""
        try:
            return (
                repo.head.commit.hexsha,
                repo.head.commit.tree.hexsha,
                "(detached)" if repo.head.is_detached else repo.active_branch.name,
                repo.head.is_detached,
            )
        except Exception:
            return "", "", "(no commits yet)", False

    def _commit(self, repo, parsed: dict) -> dict:
        result  = self._base(parsed)
        flags   = parsed["flags"]
        message = flags.get("-m") or flags.get("--message") or ""
        amend   = "--amend" in flags

        head_sha, tree_sha, branch, detached = self._head_info(repo)

        try:
            staged = list(repo.index.diff("HEAD"))
        except Exception:
            staged = []

        if amend and repo.head.commit.parents:
            parent_sha = repo.head.commit.parents[0].hexsha
        elif not amend:
            parent_sha = head_sha
        else:
            parent_sha = ""

        if not staged and not amend and "--allow-empty" not in flags:
            result["warnings"].append(
                "Nothing currently staged i.e git would abort with 'nothing to commit'."
            )

        msg_display = message or "(editor would open)"
        new_sha = self._approx_sha(
            "commit", tree_sha, parent_sha,
            self._author(repo), message or "placeholder",
        )

        action   = "Amend last commit" if amend else f"Create new commit on '{branch}'"
        msg_note = f' with message "{message}"' if message else " (no -m flag, editor opens)"
        result["interpretation"] = f"{action}{msg_note}\n{len(staged)} file(s) currently staged."
        result["objects"] = [
            self._obj(
                "commit", "create", "New commit object",
                sha_after=new_sha,
                detail=f"parent: {parent_sha[:12] or 'none'}  msg: \"{msg_display}\"",
            ),
            self._obj(
                "tree",
                "reuse" if not staged else "update",
                "Root tree",
                sha_before=tree_sha,
                sha_after=tree_sha,
                detail="unchanged" if not staged else f"{len(staged)} file(s) alter tree",
            ),
        ]

        if not detached:
            result["refs"] = [self._obj(
                "branch_ref", "update", f"refs/heads/{branch}",
                sha_before=head_sha, sha_after=new_sha,
                detail=f"{head_sha[:12]} → {new_sha[:12]}",
            )]

        result["head"]  = {"from": head_sha, "to": new_sha, "type": "branch"}
        result["index"] = [d.a_path for d in staged]
        result["note"]  = "SHA is approximate i.e actual value depends on precise commit timestamp."
        return result

    def _add(self, repo, parsed: dict) -> dict:
        result    = self._base(parsed)
        flags     = parsed["flags"]
        args      = parsed["args"]
        all_files = "-A" in flags or "--all" in flags
        update    = "-u" in flags or "--update" in flags

        try:
            if all_files:
                paths = [d.a_path for d in repo.index.diff(None)] + list(repo.untracked_files)
            elif update:
                paths = [d.a_path for d in repo.index.diff(None)]
            else:
                paths = args
        except Exception:
            paths = args

        if not paths:
            result["warnings"].append("No paths specified i.e nothing would be staged.")
            result["interpretation"] = "No files to stage."
            return result

        result["interpretation"] = (
            f"Stage {len(paths)} file path(s) to the index. "
            "A new blob object is written for each modified file "
            "if that exact content does not already exist as a blob."
        )

        for path in paths[:20]:
            full = os.path.join(repo.working_dir, path)
            try:
                data = open(full, "rb").read()
                sha  = hashlib.sha1(f"blob {len(data)}\x00".encode() + data).hexdigest()
            except Exception:
                sha = "(cannot read)"
            result["objects"].append(
                self._obj("blob", "create", os.path.basename(path),
                          sha_after=sha, detail=f"path: {path}")
            )

        if len(paths) > 20:
            result["objects"].append(
                self._obj("...", "...", f"and {len(paths) - 20} more files", "", "", "")
            )

        result["index"] = paths
        result["note"]  = "Blob SHA computed from current file content (git hash-object algorithm)."
        return result

    def _checkout(self, repo, parsed: dict) -> dict:
        result = self._base(parsed)
        flags  = parsed["flags"]
        args   = parsed["args"]
        create = "-b" in flags or "-B" in flags
        target = args[0] if args else ""

        if not target:
            result["interpretation"] = "No target specified i.e git would show current branch."
            return result

        head_sha, _, branch, _ = self._head_info(repo)

        try:
            target_sha       = repo.rev_parse(target).hexsha
            branch_names     = [b.name for b in repo.branches]
            is_detached_after = target not in branch_names
        except Exception:
            target_sha        = "(unresolvable)"
            is_detached_after = True

        if create:
            result["interpretation"] = (
                f"Create and switch to new branch '{target}' "
                f"pointing to current HEAD ({head_sha[:12]})."
            )
            result["refs"] = [self._obj(
                "branch_ref", "create", f"refs/heads/{target}",
                sha_after=head_sha, detail=f"new branch at {head_sha[:12]}",
            )]
            result["head"] = {"from": head_sha, "to": head_sha, "type": "branch"}
        else:
            detached_note = " i.e detached HEAD" if is_detached_after else ""
            result["interpretation"] = (
                f"Move HEAD to '{target}' ({target_sha[:12]}){detached_note}. "
                "Files in working directory are updated to match that snapshot."
            )
            if is_detached_after:
                result["warnings"].append(
                    "Checking out a bare SHA enters detached HEAD state. "
                    "Create a branch (-b) before making new commits."
                )
            result["head"] = {
                "from": head_sha,
                "to":   target_sha,
                "type": "detached" if is_detached_after else "branch",
            }

        result["note"] = "Working-directory file changes are not individually listed in this preview."
        return result

    _switch = _checkout

    def _reset(self, repo, parsed: dict) -> dict:
        result = self._base(parsed)
        flags  = parsed["flags"]
        args   = parsed["args"]
        target = args[0] if args else "HEAD~1"

        soft = "--soft" in flags
        hard = "--hard" in flags
        mode = "soft" if soft else ("hard" if hard else "mixed")

        try:
            target_sha = repo.rev_parse(target).hexsha
        except Exception:
            result["warnings"].append(f"Cannot resolve '{target}'.")
            result["interpretation"] = f"Cannot resolve target ref '{target}'."
            return result

        head_sha, _, branch, _ = self._head_info(repo)

        try:
            n = sum(1 for _ in repo.iter_commits(f"{target_sha}..{head_sha}"))
        except Exception:
            n = 0

        effects = {
            "soft":  "Changes from removed commits go to the index (staged, ready to re-commit).",
            "mixed": "Changes from removed commits go to the working directory (unstaged, files kept).",
            "hard":  "Changes from removed commits are permanently discarded.",
        }

        result["interpretation"] = (
            f"Move branch '{branch}' back to {target_sha[:12]}, "
            f"removing {n} commit(s) from the branch tip.\n"
            f"Mode: --{mode} - {effects[mode]}"
        )
        result["refs"] = [self._obj(
            "branch_ref", "update", f"refs/heads/{branch}",
            sha_before=head_sha, sha_after=target_sha,
            detail=f"{n} commit(s) removed  {head_sha[:12]} → {target_sha[:12]}",
        )]
        result["head"] = {"from": head_sha, "to": target_sha, "type": "branch"}

        if hard:
            result["warnings"].append(
                "Hard reset permanently overwrites working directory and index. "
                "Recovery is only possible via git reflog before garbage collection."
            )
        if n > 0:
            result["warnings"].append(
                f"{n} commit object(s) become unreachable from this branch "
                "(still in object store until git gc runs)."
            )
        result["note"] = "git reset never deletes objects i.e they persist in the object store and reflog."
        return result

    def _branch(self, repo, parsed: dict) -> dict:
        result = self._base(parsed)
        flags  = parsed["flags"]
        args   = parsed["args"]
        delete = "-d" in flags or "-D" in flags or "--delete" in flags
        force  = "-D" in flags

        if not args:
            try:
                names = [b.name for b in repo.branches]
            except Exception:
                names = []
            result["interpretation"] = f"List local branches: {', '.join(names) or '(none)'}"
            result["note"] = "Informational only i.e no changes made."
            return result

        name = args[0]

        if delete:
            try:
                sha = repo.branches[name].commit.hexsha
            except Exception:
                sha = "(unknown)"
            result["interpretation"] = f"Delete branch '{name}'."
            result["refs"] = [self._obj(
                "branch_ref", "delete", f"refs/heads/{name}",
                sha_before=sha, detail="ref file removed from .git/refs/heads/",
            )]
            if force:
                result["warnings"].append(
                    f"-D bypasses the unmerged check i.e commits unique to '{name}' "
                    "may become unreachable."
                )
        else:
            start = args[1] if len(args) > 1 else "HEAD"
            try:
                start_sha = repo.rev_parse(start).hexsha
            except Exception:
                start_sha = "(unresolvable)"
            result["interpretation"] = f"Create branch '{name}' pointing to {start_sha[:12]}."
            result["refs"] = [self._obj(
                "branch_ref", "create", f"refs/heads/{name}",
                sha_after=start_sha, detail=f"new ref at {start_sha[:12]}",
            )]

        result["note"] = "Branch operations only modify ref files i.e no git objects are created or deleted."
        return result

    def _tag(self, repo, parsed: dict) -> dict:
        result   = self._base(parsed)
        flags    = parsed["flags"]
        args     = parsed["args"]
        annotate = "-a" in flags or "--annotate" in flags
        message  = flags.get("-m") or flags.get("--message") or ""

        if not args:
            result["interpretation"] = "List all tags i.e no changes."
            result["note"] = "Informational only."
            return result

        name   = args[0]
        target = args[1] if len(args) > 1 else "HEAD"

        try:
            target_sha = repo.rev_parse(target).hexsha
        except Exception:
            target_sha = "(unresolvable)"

        if annotate:
            result["interpretation"] = (
                f"Create annotated tag '{name}' at {target_sha[:12]}. "
                "An annotated tag writes a tag object to the object store "
                "(tagger, date, message) and is recommended for releases."
            )
            result["objects"] = [self._obj(
                "tag", "create", f"Tag object: {name}",
                sha_after="(new tag object SHA)",
                detail=f"tagger: {self._author(repo)}  message: \"{message}\"",
            )]
        else:
            result["interpretation"] = (
                f"Create lightweight tag '{name}' pointing to {target_sha[:12]}. "
                "A lightweight tag is just a ref i.e no tag object is written."
            )

        result["refs"] = [self._obj(
            "tag_ref", "create", f"refs/tags/{name}",
            sha_after=target_sha, detail=f"points to {target_sha[:12]}",
        )]
        result["note"] = (
            "Annotated tags (-a) create a tag object; "
            "lightweight tags only create a ref file."
        )
        return result

    def _revert(self, repo, parsed: dict) -> dict:
        result = self._base(parsed)
        args   = parsed["args"]

        if not args:
            result["interpretation"] = "No SHA specified i.e git would show usage."
            return result

        target = args[0]
        try:
            tc         = repo.commit(target)
            target_sha = tc.hexsha
            short_msg  = tc.message.split("\n")[0][:50]
            n_files    = len(tc.stats.files)
        except Exception:
            target_sha = "(unresolvable)"
            short_msg  = ""
            n_files    = 0

        head_sha, tree_sha, branch, _ = self._head_info(repo)

        revert_msg = f'Revert "{short_msg}"'
        new_sha = self._approx_sha(
            "commit", tree_sha, head_sha, self._author(repo), revert_msg
        )

        result["interpretation"] = (
            f"Create a new commit on '{branch}' that exactly inverts:\n"
            f"  {target_sha[:12]}  \"{short_msg}\"\n"
            f"This reverses {n_files} file change(s). "
            "The original commit remains in history."
        )
        result["objects"] = [self._obj(
            "commit", "create", "New revert commit",
            sha_after=new_sha,
            detail=f"msg: '{revert_msg}'  parent: {head_sha[:12]}",
        )]
        result["refs"] = [self._obj(
            "branch_ref", "update", f"refs/heads/{branch}",
            sha_before=head_sha, sha_after=new_sha,
            detail=f"{head_sha[:12]} → {new_sha[:12]}",
        )]
        result["head"] = {"from": head_sha, "to": new_sha, "type": "branch"}
        result["note"] = "SHA is approximate i.e actual value depends on precise commit timestamp."
        return result

    def _merge(self, repo, parsed: dict) -> dict:
        result  = self._base(parsed)
        flags   = parsed["flags"]
        args    = parsed["args"]
        no_ff   = "--no-ff" in flags
        ff_only = "--ff-only" in flags

        if not args:
            result["interpretation"] = "No branch specified."
            return result

        target = args[0]
        try:
            target_sha = repo.rev_parse(target).hexsha
        except Exception:
            result["warnings"].append(f"Cannot resolve '{target}'.")
            result["interpretation"] = f"Cannot resolve '{target}'."
            return result

        head_sha, tree_sha, branch, _ = self._head_info(repo)

        try:
            bases = repo.merge_base(head_sha, target_sha)
            is_ff = bool(bases) and bases[0].hexsha == head_sha
        except Exception:
            is_ff = False

        if is_ff and not no_ff:
            result["interpretation"] = (
                f"Fast-forward '{branch}' to '{target}' i.e no merge commit needed. "
                f"'{branch}' is a direct ancestor of '{target}'; the ref simply advances."
            )
            result["refs"] = [self._obj(
                "branch_ref", "update", f"refs/heads/{branch}",
                sha_before=head_sha, sha_after=target_sha,
                detail="fast-forward i.e no new object created",
            )]
            result["head"] = {"from": head_sha, "to": target_sha, "type": "branch"}
            result["note"] = "Fast-forward: no merge commit object is written."

        elif ff_only and not is_ff:
            result["warnings"].append(
                "Branches have diverged i.e fast-forward not possible. "
                "git would abort with --ff-only."
            )
            result["interpretation"] = "Merge would be aborted (--ff-only, non-FF history)."

        else:
            ts  = int(time.time())
            msg = f"Merge branch '{target}'"
            body = (
                f"tree {tree_sha}\nparent {head_sha}\nparent {target_sha}\n"
                f"author {self._author(repo)} {ts} +0000\n"
                f"committer {self._author(repo)} {ts} +0000\n\n{msg}"
            )
            merge_sha = hashlib.sha1(
                f"commit {len(body.encode())}\x00{body}".encode()
            ).hexdigest()

            result["interpretation"] = (
                f"Three-way merge of '{target}' into '{branch}'. "
                "A merge commit with two parents will be created."
            )
            result["objects"] = [self._obj(
                "commit", "create", "Merge commit (2 parents)",
                sha_after=merge_sha,
                detail=f"parents: {head_sha[:12]},  {target_sha[:12]}",
            )]
            result["refs"] = [self._obj(
                "branch_ref", "update", f"refs/heads/{branch}",
                sha_before=head_sha, sha_after=merge_sha,
                detail=f"{head_sha[:12]} → {merge_sha[:12]}",
            )]
            result["head"] = {"from": head_sha, "to": merge_sha, "type": "branch"}
            result["warnings"].append(
                "Conflicts cannot be predicted without running the merge i.e "
                "inspect manually if branches modify the same lines."
            )
            result["note"] = "Merge commit SHA is approximate."

        return result

    def _stash(self, repo, parsed: dict) -> dict:
        result = self._base(parsed)
        sub    = parsed["args"][0] if parsed["args"] else "push"

        try:
            n_dirty  = len(list(repo.index.diff(None)))
            n_new    = len(repo.untracked_files)
            head_sha, _, branch, _ = self._head_info(repo)
        except Exception:
            n_dirty  = 0
            n_new    = 0
            head_sha = ""
            branch   = "(unknown)"

        if sub in ("push", "save", ""):
            if n_dirty == 0 and n_new == 0:
                result["warnings"].append("Working tree is clean i.e nothing to stash.")
            result["interpretation"] = (
                f"Stash {n_dirty} modified and {n_new} untracked file(s). "
                "Git creates two commit objects under refs/stash: one for the index state "
                "and one for the working-directory state. "
                "The index and working directory are then reset to HEAD."
            )
            result["objects"] = [
                self._obj("commit", "create", "Stash index commit",  sha_after="(new SHA)"),
                self._obj("commit", "create", "Stash WD commit",     sha_after="(new SHA)",
                          detail="parent: stash index commit"),
            ]
            result["refs"] = [self._obj(
                "stash_ref", "update", "refs/stash",
                sha_before="(previous top)", sha_after="(new stash entry)",
                detail=f"WIP on {branch}: {head_sha[:12]}",
            )]
            result["note"] = "Stash entries are stored as a linked list under refs/stash."

        elif sub == "pop":
            result["interpretation"] = (
                "Apply the top stash entry to the working directory and index, "
                "then remove it from the stash list."
            )
            result["note"] = (
                "Cannot fully predict outcome i.e depends on stash vs current working directory state."
            )

        elif sub == "list":
            result["interpretation"] = "Show all stash entries i.e no changes."

        elif sub == "drop":
            result["interpretation"] = "Remove the top stash entry from refs/stash."
            result["refs"] = [self._obj(
                "stash_ref", "update", "refs/stash",
                sha_before="(current top)", sha_after="(next entry)",
                detail="stash entry removed",
            )]

        return result


class CommandPreviewWidget(QWidget):
    """Input bar and collapsible results panel for git command preview."""

    def __init__(self, state: AppState):
        super().__init__()
        self._state     = state
        self._predictor = GitCommandPredictor()
        self._shown     = False

        self.setObjectName("cmdPanel")
        self.setMinimumHeight(36)

        root = QVBoxLayout(self)
        root.setContentsMargins(0, 0, 0, 0)
        root.setSpacing(0)

        bar = QWidget()
        bar.setObjectName("cmdBar")
        bar.setFixedHeight(36)
        bl = QHBoxLayout(bar)
        bl.setContentsMargins(10, 0, 8, 0)
        bl.setSpacing(8)

        prefix = QLabel("git")
        prefix.setStyleSheet(
            f"color:{ACCENT_GREEN}; font-size:12px; font-weight:700;"
            " font-family:'SF Mono','Menlo','Consolas',monospace; background:transparent;"
        )
        bl.addWidget(prefix)

        self._input = QLineEdit()
        self._input.setObjectName("cmdInput")
        self._input.setPlaceholderText(
            'commit -m "msg"  ·  add src/  ·  reset --soft HEAD~1  ·  checkout main  ·  merge feature'
        )
        self._input.returnPressed.connect(self._run)
        bl.addWidget(self._input)

        self._run_btn = QPushButton("Preview")
        self._run_btn.setObjectName("cmdPreviewBtn")
        self._run_btn.setFixedHeight(26)
        self._run_btn.clicked.connect(self._run)
        bl.addWidget(self._run_btn)

        self._toggle_btn = QPushButton("Show")
        self._toggle_btn.setObjectName("cmdToggleBtn")
        self._toggle_btn.setFixedSize(42, 26)
        self._toggle_btn.clicked.connect(self._toggle)
        bl.addWidget(self._toggle_btn)

        root.addWidget(bar)

        self._tree = QTreeWidget()
        self._tree.setObjectName("cmdTree")
        self._tree.setHeaderHidden(True)
        self._tree.setRootIsDecorated(True)
        self._tree.setIndentation(18)
        self._tree.setAnimated(True)
        self._tree.hide()
        root.addWidget(self._tree)

        state.repo_changed.connect(lambda _: self._tree.clear())

    def _run(self):
        raw = self._input.text().strip()
        if not raw:
            return
        if not raw.lower().startswith("git ") and raw.lower() != "git":
            raw = "git " + raw

        parsed = GitCommandParser.parse(raw)
        if parsed is None:
            self._show_error("Cannot parse command i.e check for unclosed quotes.")
            return

        result = self._predictor.predict(self._state.repo, parsed)
        self._render(result)

        if not self._shown:
            self._toggle()

        self._state.logger.log(
            f"Command preview: {raw[:60]}  →  "
            f"{'supported' if result['supported'] else 'unsupported'}"
        )

    def _toggle(self):
        self._shown = not self._shown
        self._tree.setVisible(self._shown)
        self._toggle_btn.setText("Hide" if self._shown else "Show")

    def _render(self, result: dict):
        self._tree.clear()

        action_colors = {
            "create": ACCENT_GREEN,
            "update": ACCENT_ORANGE,
            "delete": ACCENT_RED,
            "reuse":  TEXT_TERTIARY,
        }

        interp_rows = [("Interpretation", result["interpretation"], TEXT_PRIMARY)]
        if not result["supported"]:
            self._section("Command", result["command"], interp_rows, expand=True)
            return

        interp_rows += [
            ("Verb",      result["verb"], TEXT_SECONDARY),
            ("Supported", "Yes",          ACCENT_GREEN),
        ]
        self._section("Command", result["command"], interp_rows, expand=True)

        obj_rows = []
        for obj in result.get("objects", []):
            sha = (
                f"{obj['sha_before'][:10]} → {obj['sha_after'][:10]}"
                if obj["sha_before"] and obj["sha_after"] else
                f"new: {obj['sha_after'][:12]}" if obj["sha_after"] else ""
            )
            color = action_colors.get(obj["action"], TEXT_SECONDARY)
            text  = f"{obj['type'].upper():<8}  [{obj['action']}]  {obj['name']}"
            obj_rows.append((text, f"{sha}  {obj['detail']}", color))

        self._section(
            f"Objects  ({len(obj_rows)})", None,
            obj_rows or [("No new objects created", "", TEXT_TERTIARY)],
            expand=bool(obj_rows),
        )

        ref_rows = []
        for ref in result.get("refs", []):
            color = action_colors.get(ref["action"], TEXT_SECONDARY)
            text  = f"{ref['type'].replace('_ref', '').upper():<10}  [{ref['action']}]  {ref['name']}"
            ref_rows.append((text, ref["detail"], color))

        self._section(
            f"References  ({len(ref_rows)})", None,
            ref_rows or [("No ref changes", "", TEXT_TERTIARY)],
            expand=bool(ref_rows),
        )

        head = result.get("head", {})
        if head and (head.get("from") or head.get("to")):
            frm, to, typ = head.get("from", ""), head.get("to", ""), head.get("type", "")
            if frm == to:
                head_rows = [("HEAD stays", f"{frm[:12]}  ({typ})", TEXT_SECONDARY)]
            else:
                head_rows = [(
                    "HEAD moves",
                    f"{frm[:12] or 'none'} → {to[:12] or 'none'}  [{typ}]",
                    ACCENT_ORANGE,
                )]
        else:
            head_rows = [("HEAD does not move", "", TEXT_TERTIARY)]

        self._section("HEAD", None, head_rows, expand=True)

        idx      = result.get("index", [])
        idx_rows = [(f"  {p}", "would be staged", TEXT_SECONDARY) for p in idx[:15]]
        if len(idx) > 15:
            idx_rows.append((f"  ... and {len(idx) - 15} more", "", TEXT_TERTIARY))
        self._section(
            f"Staging  ({len(idx)})", None,
            idx_rows or [("No index changes", "", TEXT_TERTIARY)],
            expand=False,
        )

        warn_rows = [(w, "", ACCENT_ORANGE) for w in result.get("warnings", [])]
        if result.get("note"):
            warn_rows.append((result["note"], "", TEXT_TERTIARY))
        if warn_rows:
            self._section(f"Warnings  ({len(warn_rows)})", None, warn_rows, expand=True)

        foot = QTreeWidgetItem(self._tree)
        foot.setText(0, "  Read-only preview.")
        foot.setForeground(0, QColor(TEXT_TERTIARY))
        font = foot.font(0)
        font.setItalic(True)
        foot.setFont(0, font)

    def _section(self, title: str, subtitle: "str | None",
                 rows: list, expand: bool = True):
        sect = QTreeWidgetItem(self._tree)
        sect.setText(0, title)
        font = sect.font(0)
        font.setBold(True)
        font.setPointSize(11)
        sect.setFont(0, font)
        sect.setForeground(0, QColor(TEXT_PRIMARY))

        if subtitle:
            sub = QTreeWidgetItem(sect)
            sub.setText(0, f"   {subtitle}")
            sub.setForeground(0, QColor(TEXT_SECONDARY))

        for text, detail, color in rows:
            row = QTreeWidgetItem(sect)
            row.setText(0, f"   {text}{'    ' + detail if detail else ''}")
            row.setForeground(0, QColor(color))

        sect.setExpanded(expand)

    def _show_error(self, msg: str):
        self._tree.clear()
        item = QTreeWidgetItem(self._tree)
        item.setText(0, msg)
        item.setForeground(0, QColor(ACCENT_RED))
        if not self._shown:
            self._toggle()